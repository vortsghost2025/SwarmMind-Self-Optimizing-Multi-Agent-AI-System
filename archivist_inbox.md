...
| src/attestation/Verifier.js:220‑232 | getMigrationStatus() has been REMOVED. JWS-only enforcement is the only mode. Constructor rejects allowLegacy/hmacCutoffDate options with hard error. | HMAC fallback is not merely disabled — it cannot be enabled. |
...
5. Helper methods – getMigrationStatus() removed. JWS-only enforcement is the sole mode. Constructor hard-rejects allowLegacy and hmacCutoffDate options.
